...
// 2D Forward FFT
//----------------------------------------------------------
void ForwardFFT(const cv::Mat& Src, Mat* FImg, bool shift = true)
{
...
float LogPolar(Mat& src, Mat& dst)
{
    ...
    // Radius cache.
    std::vector<float> radii(src.cols);
    ...
    for (int i = 0; i < angles.size(); ++i) {
        ...
        for (int j = 0; j < radii.size(); ++j) {
            const float& r = radii[j];
            float x = r * s + center.x;
            float y = r * c + center.y;
...
RotatedRect LogPolarFFTTemplateMatch(Mat& im0, Mat& im1, double canny_threshold1, double canny_threshold2)
{
    ...
    double mnv, mxv;
    cv::namedWindow("f0", cv::WINDOW_NORMAL);
    cv::minMaxIdx(f0, &mnv, &mxv);
    cv::imshow("f0", f0 / mxv);
    //cv::imshow("f1lp", f1lp/mxv);
